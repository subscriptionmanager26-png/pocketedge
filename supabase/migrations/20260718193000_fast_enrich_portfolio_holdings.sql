-- Fix: enrich_portfolio_holdings was ~2.4s for a 50-holding portfolio because
-- each holding ran a lateral join over social_market_assets + latest_dates.
-- Table is unique on (asset_type, asset_key), so portfolio enrich can hash-join
-- by ticker/ISIN and prefer live quotes — no per-row date filtering needed.
