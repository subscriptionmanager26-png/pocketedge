-- AMFI publishes NAVs per scheme, not as one atomic market-wide snapshot.
-- A valid fund NAV can therefore be a day behind the newest scheme NAV.
-- Keep funds visible regardless of their individual NAV date; retain strict
-- latest-date filtering for exchange-traded assets.
...
as $$
  ...
  select json_build_object(
    'asset_type', a.asset_type, 'asset_key', a.asset_key, 'name', a.name,
    'price', a.price, 'change_pct', a.change_pct, 'previous_close', a.previous_close,
    'as_of_date', a.as_of_date, 'price_source', a.price_source, 'synced_at', a.synced_at,
    'exchange', a.exchange, 'exchange_symbol', a.exchange_symbol, 'isin', a.isin
  )
  ...
  where (a.asset_type = 'fund' or a.as_of_date = d.as_of_date)
    and (
      (a.asset_type in ('stock', 'etf', 'commodity', 'bond') and a.asset_key = upper(trim(p_key)))
...
begin
  ...
  select json_build_object(
    'items', coalesce(json_agg(json_build_object(
      'asset_type', r.asset_type, 'asset_key', r.asset_key, 'name', r.name,
      'price', r.price, 'change_pct', r.change_pct, 'previous_close', r.previous_close,
      'as_of_date', r.as_of_date, 'price_source', r.price_source, 'synced_at', r.synced_at,
      'exchange', r.exchange, 'exchange_symbol', r.exchange_symbol, 'isin', r.isin,
      'score', r.score
...
begin
  ...
  from (
    select distinct on (norm_key)
      ...
      a.as_of_date, a.price_source, a.synced_at, a.exchange, a.exchange_symbol, a.isin
    from (
