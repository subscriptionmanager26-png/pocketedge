 ...
 * maker → portfolio name → thesis (~140 chars / 3 lines) → 1D
 *
 * Layout uses shrink-0 regions so text never overlaps the footer.
 */
export default function IdeaCard({
  ...
  return (
    <div
      ...
      className="grid h-[228px] w-full cursor-pointer grid-rows-[auto_auto_1fr_auto] gap-0 overflow-hidden rounded-xl border border-pe-border-strong bg-pe-surface p-4 text-left transition hover:bg-white"
    >
      <p className="truncate text-[15px] font-semibold leading-6 tracking-tight text-pe-text">
        <span
          role="link"
          tabIndex={0}
          onClick={openMaker}
      ...
      <p className="mt-1 truncate text-[12px] leading-4 text-pe-text-muted">{portfolioName}</p>

      <p className="mt-3 line-clamp-3 self-start text-[15px] leading-6 text-pe-text-secondary">
        {thesis || <span className="italic text-pe-text-muted">No thesis yet</span>}
      </p>

      <div className="mt-3 border-t border-pe-border pt-3">
        ...
        {blurReturns ? (
          ...
          <p className={`mt-1 h-7 text-[20px] font-semibold tabular-nums leading-7 ${ideaReturnClass(dayReturn)}`}>
