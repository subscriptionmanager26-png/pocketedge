...
export default function ScreenshotStep({ onBack, onSubmit }) {
  ...
  const handleNext = async () => {
    if (!shots.length) {
      setError('Add at least one Zerodha holdings screenshot.');
      return;
    }
    ...
    try {
      ...
      if (!parsed.length) {
        throw new Error('No holdings found. Use clear Zerodha holdings screenshots.');
      }
      setRows(parsed);
      setPhase('review');
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-[15px] leading-relaxed text-pe-text-secondary">
        Add one or more Zerodha Kite holdings screenshots. Parsing runs on your
      ...
      <div className="mt-8 border-t border-pe-border pt-8">
        ...
        <ol className="mt-3 list-decimal space-y-1.5 pl-5 text-[15px] text-pe-text-secondary">
          <li>Zerodha Kite → Holdings (mobile or desktop)</li>
