/** Strip suffix and map to a consistent short AMC label. */
...
function stripMutualFundSuffix(amc) {
  return amc
    .trim()
...
function normalizeBaseName(amc) {
  const stripped = stripMutualFundSuffix(amc);
  if (!stripped) return '—';

  const displayHit = Object.keys(AMC_DISPLAY).find(
  ...
  if (stripped === stripped.toLowerCase() || stripped === stripped.toUpperCase()) {
    return stripped.split(' ').map(titleCaseWord).join(' ');
  }

  return stripped;
}

export function standardizeAmcName(amc) {
  if (!amc?.trim()) return '—';
  const base = normalizeBaseName(amc);
  return base === 'Kotak Mahindra' ? 'Kotak' : (AMC_DISPLAY[base] ?? base);
}
