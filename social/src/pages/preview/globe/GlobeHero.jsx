...
const FRAGMENT_SHADER_LIGHT = `
  ...
  void main() {
    ...
    base = mix(base, base * vec3(1.04, 1.03, 1.0), 0.45);
    vec3 viewDir = normalize(vViewPosition);
    float rim = 1.0 - max(dot(viewDir, vNormal), 0.0);
    float atmosphere = smoothstep(0.5, 0.92, rim);
    vec3 atmosphereColor = vec3(1.0, 0.404, 0.098);
    vec3 finalColor = base + atmosphereColor * atmosphere * 0.18;
    gl_FragColor = vec4(finalColor, 1.0);
  }
`;
...
export default function GlobeHero({ theme = 'light' }) {
  ...
  useEffect(() => {
    ...
    const earthMaterial = new THREE.ShaderMaterial({
      ...
      fragmentShader: isLight ? FRAGMENT_SHADER_LIGHT : FRAGMENT_SHADER_DARK,
    });

    new THREE.TextureLoader().load(
      isLight ? EARTH_TEXTURE_DAY : EARTH_TEXTURE_NIGHT,
      (texture) => {
    ...
    const allNodes = [];
    const labeledNodes = [];
    const nodeColor = isLight ? 0xff6719 : 0xffffff;
    const nodeMaterial = new THREE.MeshBasicMaterial({ color: nodeColor });

    const labelClass = isLight
      ? 'pe-globe-city-label pe-globe-city-label--light'
      : 'pe-globe-city-label';
    ...
    const arcMaterial = new THREE.LineBasicMaterial({
      color: isLight ? 0xff6719 : 0xffffff,
      transparent: true,
    ...
    const particlesMesh = new THREE.Points(
      ...
      new THREE.PointsMaterial({
        size: 0.05,
        color: isLight ? 0x8a8a8a : 0xffffff,
        transparent: true,
    ...
    const updateLabels = () => {
      ...
      labeledNodes.forEach((node) => {
        ...
        node.element.style.opacity = cameraToNode.dot(normal) > -0.05 ? '0' : '1';
      });
    };
