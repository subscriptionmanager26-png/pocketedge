...
export function mergeSchemeFromCatalog(
  ...
  return {
    ...
    isin: pickIsin(row.isin, fallback.isin),
...
export async function holdingsPortfolioUrlForRow(
  ...
  const latest = normalizeAsOf(String(row.latest_as_of || row.as_of || ''));
  if (latest) {
    return holdingsPortfolioUrl(portfolioId, latest);
  }
  return holdingsPortfolioUrl(portfolioId, null);
}
...
function buildIsinIndex(
  ...
  for (const [amfi, row] of Object.entries(catalog)) {
    const isin = normalizeFundIsin(String(row.isin || ''));
...
export async function lookupCatalogByIsin(
  ...
  return isinIndexMemory.data[isin] ?? null;
}
